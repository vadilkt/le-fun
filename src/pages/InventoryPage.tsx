...
type MenuItem = {
  ...
  name: string;
  price: number;
  products: number[]; // ids des produits inclus
};
...
export function InventoryPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (!supabase) {
        // Mode démo sans Supabase configuré: on garde la logique en mémoire vide.
        setLoading(false);
        return;
      }
  ...
  const saveMenu = async (data: Omit<MenuItem, 'id'>, id?: number) => {
    ...
    try {
      ...
      // recharger menus
      const [{ data: menusData, error: mError }, { data: menuItemsData, error: miError }] =
        await Promise.all([
          supabase.from('menus').select('id,name,price').order('name'),
          supabase.from('menu_items').select('id,menu_id,product_id'),
        ]);
      if (mError || miError) throw mError || miError;
                  ...
                  <tr key={p.id}>
                    ...
                    <td>
                      <span
                        ...
                        style={{
                          background: isLow ? '#fee2e2' : '#ecfdf3',
                        }}
                      >
                        <span
        ...
        <table className="table">
          ...
          <tbody>
            {loading && menus.length === 0 ? (
              <tr>
                <td colSpan={4}>
                  <span className="muted">Chargement des menus...</span>
                </td>
              </tr>
            ) : (
              menus.map((m) => {
...
function ProductFormModal({ product, onCancel, onSave }: ProductFormModalProps) {
...
function MenuFormModal({ menu, products, onCancel, onSave }: MenuFormModalProps) {
  const [name, setName] = useState(menu?.name ?? '');
  const [price, setPrice] = useState(menu?.price ?? 0);
  const [selectedIds, setSelectedIds] = useState<number[]>(
    menu?.products ?? (products[0] ? [products[0].id] : [])
  );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    onSave({ name, price, products: selectedIds }, menu?.id);
  };
                ...
                return (
                  <label
                    ...
                    style={{
                      ...
                      borderRadius: 999,
                      cursor: 'pointer',
                      background: checked ? '#f9731615' : 'transparent',
                    }}
                  >
                    <input
                      type="checkbox"
